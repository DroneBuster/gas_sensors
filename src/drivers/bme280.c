...
bool write_register(i2caddr_t addr, uint8_t regaddr, uint8_t* data, uint8_t tx_len);
...
bool write_register(i2caddr_t addr, uint8_t regaddr, uint8_t* data, uint8_t tx_len)
{
  ...
  send_buff[0] = regaddr;
  send_buff[1] = data[0];
  res = i2cMasterTransmitTimeout(&BME280_BUS, addr, send_buff, 2, NULL, 0, MS2ST(1000));
  if(res != 0)
    return false;
